...
class Accommodation(models.Model):
    ...
    main_photo = models.ImageField(
        upload_to=get_accommodations_path, blank=True, null=True)
    ...
    def create_thumbnails(self):
        ...
        DJANGO_TYPE = self.main_photo.file.name.split(
            "/")[-1].split(".")[-1].lower()
        ...
        self.main_photo.save('%s_big.%s' % (os.path.splitext(suf_big.name)[
...
class AccommodationPhoto(models.Model):
    ...
    photo = models.ImageField(
        upload_to=get_accommodation_photos_path, blank=True, null=True)
    accommodation = models.ForeignKey(
        Accommodation, related_name="booking_accommodation")
    ...
    def save(self):
        if AccommodationPhoto.objects.filter(accommodation=self.accommodation).count() >= 4:
            raise Exception(
                "Too many Photos! Delete one! ( max 4 photos per Accommodation )")
            return None
        # create a thumbnail
        self.create_thumbnails()
...
class Booking(models.Model):
    ...
    status = models.CharField(
        max_length=3, default="WFA", choices=BOOKING_STATUSES)
    tenant = models.ForeignKey(User, related_name="booking_tenant")
    accommodation = models.ForeignKey(
        Accommodation, related_name="booking_of_accommodation")
